...
export default function SearchResults() {
    ...
    useEffect(() => {
        const fetchResults = async () => {
            ...
            setError(false);
            try {
                // On récupère les params "simples" depuis l'URL
                const filters = {
    ...
    return (
        <MainLayout>
            <div className="space-y-6 animate-fade-in min-h-[80vh]">
                ...
                {loading ? (
                    <Loader text="Interrogation du serveur..." />
                ) : error ? (
                    <EmptyState
                    ...
                    <EmptyState
                        ...
                        icon={<Filter className="w-10 h-10 text-gray-300" />}
                    />
                ) : (
                        ...
                        {results.map(ride => (
                            <Card key={ride.id} className="flex flex-col justify-between hover:shadow-lg transition-shadow border-emerald-100/50">
                                ...
                                <div className="p-4 bg-gray-50 border-t border-gray-100 mt-auto">
                                    <Button className="w-full btn-sm" onClick={() => openBookingPopup(ride)}>
                                        Réserver
                                    </Button>
                                </div>
                            </Card>
